...
import {MeltQuoteState} from '#server/modules/cashu/cashu.enums';
...
export class MintMeltQuoteResolver {
	...
	async mint_melt_quotes(
		...
		return await this.mintMeltQuoteService.getMintMeltQuotes(tag, {
	...
	async mint_nut05_update(
		...
		this.logger.debug(tag);
		return await this.mintMeltQuoteService.updateMintNut05(tag, normalizeMintUnit(unit), method, disabled, min_amount, max_amount);
	}

...
#                              address (cln-cdk only; requires mint NUT-30 / BDK)
#   - ACTIVITY_ONCHAIN_MELTS — wallet melts to an on-chain address (cln-cdk only)
#
# Requires:
...
# ───── 9. Wallet onchain mints (NUT-30 / BDK backend) ─────
...
# ───── 10. Wallet onchain melts (NUT-30 / BDK backend) ─────
...
# ───── 11. Mempool fill — varied-fee unconfirmed self-sends ─────
...
log "DONE — forwards=$ACTIVITY_FORWARDS inbound=$ACTIVITY_INBOUND outbound=$ACTIVITY_OUTBOUND mints=$ACTIVITY_MINTS swaps=$ACTIVITY_SWAPS melts=$ACTIVITY_MELTS bolt12_mints=$ACTIVITY_BOLT12_MINTS bolt12_melts=$ACTIVITY_BOLT12_MELTS onchain_mints=$ACTIVITY_ONCHAIN_MINTS onchain_melts=$ACTIVITY_ONCHAIN_MELTS mempool=$((ACTIVITY_MEMPOOL_PER_RATE * 6))"
